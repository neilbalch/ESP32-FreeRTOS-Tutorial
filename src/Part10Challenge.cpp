...
void eat(void* param) {
  ...
  while(1) {
    ...
    if(xSemaphoreTake(chopsticks[num], mutex_timeout) == pdFALSE) {
      sprintf(buf, "Philosopher %i timeed out while trying to take chopstick %i", num, num);
      Serial.println(buf);

      // Release kernel objects (none taken yet) and return to the top of the loop later
    ...
    if(xSemaphoreTake(chopsticks[(num + 1) % NUM_TASKS], mutex_timeout) == pdFALSE) {
      sprintf(buf, "Philosopher %i timeed out while trying to take chopstick %i", num, (num + 1) % NUM_TASKS);
      Serial.println(buf);
  ...
  // Decide which of the chopsticks available to the philosopher is lower in the hierarchy (index in this case)
  if(right < left) {
    // Take right chopstick
