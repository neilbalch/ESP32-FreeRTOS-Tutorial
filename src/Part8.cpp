...
void setup8() {
  ...
  one_shot_timer = xTimerCreate("One-shot timer",           // Timer name
  ...
  auto_reload_timer = xTimerCreate("Auto-reload timer",       // Timer name
  ...
  if(one_shot_timer == NULL || auto_reload_timer == NULL) Serial.println("Couldn't create timers!");
  else {
    // Wait and then start timer
    vTaskDelay(1000 / portTICK_PERIOD_MS);
    Serial.println("Starting timers...");
    // Will start the timer immediately or wait until the command queue has a free slot
    // Recall that commands to the timer task go into an execution queue
